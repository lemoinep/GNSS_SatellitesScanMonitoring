...
# GOAL:
# This program connects to a GPS receiver (Yocto-GPS or system GPS) and determines the user's current geographic position.
# It retrieves and parses satellite data to calculate the elevation and azimuth of each visible satellite.
# For each satellite, it estimates its approximate latitude, longitude, and altitude in space.
# The program can display this information in the console and, with the 3D option, 
# shows the satellites' positions on an interactive celestial sphere.
# Cardinal points and satellite names are visualized, and the user can rotate the sky map with the mouse
...
def update_satellites(scene, satellites_data):
    ...
    # Remove old satellites if count changed
    if len(scene.sat_objects) > len(satellites_data):
        for obj in scene.sat_objects[len(satellites_data):]:
            obj['sphere'].visible = False
            obj['label'].visible = False
        scene.sat_objects = scene.sat_objects[:len(satellites_data)]
    
    # Create/update satellite objects
    for i, sat in enumerate(satellites_data):
        ...
        if i < len(scene.sat_objects):
            # Update existing satellite
            scene.sat_objects[i]['sphere'].pos = vector(x, y, z)
            scene.sat_objects[i]['label'].pos = vector(x, y, z)
            scene.sat_objects[i]['label'].text = f"{sat['id']}\n{sat['name']}"
        else:
